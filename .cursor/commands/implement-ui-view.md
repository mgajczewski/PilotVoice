...
1. Plan implementacji (np. generations-view-implementation-plan).
2. Definicje typów (np. plik types).
3. Zasady implementacji (np. shared, frontend, astro, react, ui-shadcn-helper).
Koniecznie dopytaj o te 3 rzeczy w podany wyżej sposób (zasugeruj wymienione przykłady), zanim rozpoczniesz tworzenie planu.

Następnie wdrażaj plan zgodnie z następującym podejściem:
